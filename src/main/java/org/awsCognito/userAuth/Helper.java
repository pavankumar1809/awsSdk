...
public class Helper {
	...
	Helper(String userPoolID, String clientid, String secretKey) {
		do {
	 ...
	 */
	public RespondToAuthChallengeResult PerformSRPAuthentication(String username, String password) {
		RespondToAuthChallengeResult authresult = null;
...
//                System.out.println(CognitoJWTParser.getPayload(result.getAuthenticationResult().getIdToken()));
				authresult = result;
				authresult.addChallengeParametersEntry("SECRET_HASH",
	...
	private RespondToAuthChallengeRequest userSrpAuthRequest(InitiateAuthResult challenge, String password) {
		...
		if (B.mod(Helper.N).equals(BigInteger.ZERO)) {
