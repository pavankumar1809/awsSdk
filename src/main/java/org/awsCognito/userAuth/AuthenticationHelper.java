...
class AuthenticationHelper {
	 ...
	 */
	public RespondToAuthChallengeResponse PerformSRPAuthentication(String username, String password) {
		RespondToAuthChallengeResponse authresult = null;
		...
		try {
			...
			if (ChallengeNameType.PASSWORD_VERIFIER.toString().equals(initiateAuthResult.challengeName().toString())) {
...
//                System.out.println(CognitoJWTParser.getPayload(result.getAuthenticationResult().getIdToken()));
				authresult = result;
				authresult.challengeParameters().put("SECRET_HASH",
						initiateAuthRequest.authParameters().get("SECRET_HASH"));
			}
	...
	private InitiateAuthRequest initiateUserSrpAuthRequest(String username) {
		...
		if (secretKey != null && !secretKey.isEmpty()) {
			authparams.put("SECRET_HASH", calculateSecretHash(clientId, secretKey, username));
		}
		authparams.put("CHALLENGE_NAME", "SRP_A");
		authparams.put("SRP_A", this.getA().toString(16));

		InitiateAuthRequest initiateAuthRequest = InitiateAuthRequest.builder().authFlow(AuthFlowType.CUSTOM_AUTH)
	...
	private RespondToAuthChallengeRequest userSrpAuthRequest(InitiateAuthResponse challenge, String password,
		...
		RespondToAuthChallengeRequest authChallengeRequest = RespondToAuthChallengeRequest.builder()
				.challengeName(challenge.challengeName()).clientId(clientId).session(challenge.session())
